...
# JWT RS256 keys — generate with:
#   openssl genrsa -out jwt_signing.pem 2048
#   openssl rsa -in jwt_signing.pem -pubout -out jwt_verifying.pem
# Then set:
#   JWT_SIGNING_KEY_FILE=/run/secrets/jwt_signing_key
#   JWT_VERIFYING_KEY_FILE=/run/secrets/jwt_verifying_key
JWT_SIGNING_KEY_FILE=
JWT_VERIFYING_KEY_FILE=
...
ENV
        ...
        log "IMPORTANT: Edit .env to set JWT key paths, FRONTEND_URL, and KAFKA_EXTERNAL_HOST"
    else
...
Next steps:
  1. Edit ${SEQUOIA_DIR}/.env
     - Set JWT_SIGNING_KEY_FILE and JWT_VERIFYING_KEY_FILE
