...
JWT_SIGNING_KEY_FILE=${SEQUOIA_DIR}/jwt_signing.pem
JWT_VERIFYING_KEY_FILE=${SEQUOIA_DIR}/jwt_verifying.pem
...
ENV
        ...
        if [ ! -f "${SEQUOIA_DIR}/jwt_signing.pem" ]; then
            openssl genrsa -out "${SEQUOIA_DIR}/jwt_signing.pem" 2048 2>/dev/null
            openssl rsa -in "${SEQUOIA_DIR}/jwt_signing.pem" -pubout \
                -out "${SEQUOIA_DIR}/jwt_verifying.pem" 2>/dev/null
            chmod 600 "${SEQUOIA_DIR}/jwt_signing.pem" "${SEQUOIA_DIR}/jwt_verifying.pem"
