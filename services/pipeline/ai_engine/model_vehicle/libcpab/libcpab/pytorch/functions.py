...
#%%
def to(x, dtype=torch.float32, device=None, device_name='cuda:0'):
    if type(device)==str:
        device = torch.device(device_name) if device=="gpu" else torch.device("cpu")
...
def sample_transformation(d, n_sample=1, mean=None, cov=None, device='cpu', device_name='cuda:0'):
    device = torch.device(
...
def identity(d, n_sample=1, epsilon=0, device='cpu', device_name='cuda:0'):
    assert epsilon>=0, "epsilon need to be larger than 0"
    device = torch.device('cpu') if device=='cpu' else torch.device(device_name)
    return torch.zeros(n_sample, d, dtype=torch.float32, device=device) + epsilon

#%%
def uniform_meshgrid(ndim, domain_min, domain_max, n_points, device='cpu',device_name='cuda:0'):
    device = torch.device(
        'cpu') if device == 'cpu' else torch.device(device_name)
