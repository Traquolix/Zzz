...
def interpolate1D(data, grid, outsize):
    ...
    # Interpolation weights - fix tensor size mismatch
    xd = (x-x0.to(torch.float32))
    # Ensure xd matches the shape of c0 and c1 for broadcasting
    if xd.numel() != c0.shape[0]:
        # Adjust sizes to match - take minimum to avoid index errors
        min_size = min(xd.numel(), c0.shape[0])
        xd = xd[:min_size]
        c0 = c0[:min_size]
        c1 = c1[:min_size]
    xd = xd.reshape((-1,1))
    ...
    # Reshape - handle adjusted tensor sizes
    actual_width = c.shape[0] // n_batch
    new_data = torch.reshape(c, (n_batch, actual_width, n_channels))
    new_data = new_data.permute(0, 2, 1)

    # If output size doesn't match expected, pad or trim
    if actual_width != out_width:
        if actual_width < out_width:
            # Pad with zeros
            padding = torch.zeros(n_batch, n_channels, out_width - actual_width,
                                device=new_data.device, dtype=new_data.dtype)
            new_data = torch.cat([new_data, padding], dim=2)
        else:
            # Trim to expected size
            new_data = new_data[:, :, :out_width]
