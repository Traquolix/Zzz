...
from .core.utility import create_dir, get_dir, params
...
class Cpab(object):
    ...
    def uniform_meshgrid(self, n_points):
        ...
        return self.backend.uniform_meshgrid(self.params.ndim,
                                             ...
                                             n_points, self.device, self.device_name)
    ...
    def sample_transformation_with_prior(self, n_sample=1, mean=None,
        ...
        centers = self.backend.to(self.tesselation.get_cell_centers(), device=self.device, device_name=self.device_name)
        ...
        cov_init = self.backend.zeros(
            self.params.D, self.params.D, device=self.device_name)
    ...
    #%%
    def transform_data(self, data, theta, outsize, return_transformed_grid=False):
