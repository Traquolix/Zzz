...
class CpabAligner(object):
    ...
    def alignment_by_sampling(self, x1, x2, maxiter=100):
        ...
        for i in tqdm(range(maxiter), desc='Alignment of samples', unit='samples'):
            ...
            theta = 1e-1 * \
                self.T.sample_transformation(1, mean=current_sample.flatten())
    ...
    def alignment_by_gradient(self, x1, x2, maxiter=100, lr=1e-2):
        ...
        self.T._check_type(x2)
        assert x1.shape == x2.shape, ' Two data points does not have the same shape '
        outsize = (x2.shape[2], x2.shape[1])  # My modif
        uniform_grid = self.T.uniform_meshgrid(outsize)
        # outsize = (x2.shape[1], x2.shape[2]) if self.T.backend_name != 'pytorch' else  \
        #     (x2.shape[2], x2.shape[3])
        ...
        if self.T.backend_name == 'pytorch':
            ...
            theta = torch.autograd.Variable(
                self.T.identity(1, epsilon=1e-6), requires_grad=True)
            # theta = torch.autograd.Variable(torch.rand(1, self.T.get_theta_dim(), dtype=torch.float32,
            #                                             device="cpu"), requires_grad=True)  # My modif
            # print(theta)
            ...
            for i in range(maxiter):
                ...
                x1_trans = self.T.transform_data(
                    x1, theta, outsize=outsize)  # My modif
                grid_trans = self.T.transform_grid(
                    uniform_grid, theta)
            ...
            return theta, x1_trans, uniform_grid, grid_trans  # My modif
            # return theta
