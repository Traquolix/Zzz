...
# Platform-specific reference files: float32 accumulation differs between
# ARM (macOS) and x86 (Linux server), producing ~2% divergence in GLRT arrays.
# Each platform has its own reference generated by regenerate_reference.py.
...
def _check_fixtures():
    ...
    if not GOLDEN_REF.exists():
        missing.append(f"{GOLDEN_REF} (run: make snapshot-confirm on {_ARCH})")
    if missing:
