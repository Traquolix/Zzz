      ...
      <div className="flex items-center justify-between mb-3">
        ...
        {isDirty && (
          <button
            ...
            className="px-2.5 py-1 rounded text-cq-2xs font-medium bg-[var(--dash-accent)] text-white cursor-pointer hover:opacity-80 transition-opacity"
          >
            Apply
          </button>
        )}
        ...
        {fields.map(f => (
          <label key={f.key} className="flex items-center gap-2">
            ...
            <input
              ...
              className="w-12 px-1 py-0.5 rounded bg-transparent border border-[var(--dash-border)] text-cq-xs text-[var(--dash-text)] text-center outline-none focus:border-[var(--dash-text-secondary)] [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
            />
            <span className="text-cq-2xs text-[var(--dash-text-muted)]">km/h</span>
          </label>
        ))}
      </div>
    </div>
