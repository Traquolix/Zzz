...
export function useMapHighlights({
  ...
  const clearHighlight = useCallback(() => {
    ...
    if (highlightedMarkerRef.current) {
      highlightedMarkerRef.current.style.animation = 'dash-incident-ring 2s ease-in-out infinite'
  ...
  const highlightIncident = useCallback(
    (incidentId: string) => {
      ...
      dot.style.animation = 'dash-incident-ring 0.6s ease-in-out infinite'
    },
  ...
  const highlightChannel = useCallback(
    (lng: number, lat: number) => {
      ...
      el.style.cssText = `
        ...
        animation: dash-channel-pulse 2s ease-in-out infinite;
      `
