      ...
      {!visible || transitioning ? (
        ...
        width > 0 && (
          <LineChart data={data} width={width} height={chartHeight} margin={{ top: 4, right: 4, bottom: 4, left: 4 }}>
            <CartesianGrid stroke="var(--dash-chart-grid, rgba(255,255,255,0.03))" strokeDasharray="3 3" />
            <XAxis
              dataKey="time"
            ...
            {incidentTime && (
              <ReferenceLine
                x={incidentTime}
                stroke="var(--dash-red, #ef4444)"
                strokeDasharray="4 3"
                strokeWidth={1.5}
                label={{ value: 'Incident', position: 'top', fill: 'var(--dash-red, #ef4444)', fontSize: 9 }}
              />
            )}
            <Line
              type="monotone"
...
function ChartSkeleton() {
  return <div className="w-full h-full rounded-lg bg-[var(--dash-surface-raised)] animate-pulse" />
}
