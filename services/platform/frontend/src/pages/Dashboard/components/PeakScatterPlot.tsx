...
export function PeakScatterPlot({ data }: { data: PeakFrequencyData }) {
  ...
  const clipId = `dash-scatter-${rawId}`
    ...
    <div ref={containerRef} className="w-full relative">
      {zoom && (
        <button
          ...
          className="absolute top-0 right-0 z-10 flex items-center gap-1 px-2 py-1 text-cq-2xs text-[var(--dash-text-muted)] hover:text-[var(--dash-text)] rounded transition-colors cursor-pointer"
        >
      ...
      {tooltip && !brush && (
        <div
          className="absolute bg-[var(--dash-surface-raised)] text-[var(--dash-text)] text-cq-2xs px-2 py-1.5 rounded shadow-lg pointer-events-none z-10 whitespace-nowrap border border-[var(--dash-border)]"
          style={{
