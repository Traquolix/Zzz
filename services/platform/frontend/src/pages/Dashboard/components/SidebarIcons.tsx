  ...
  return (
    <button
      ...
      className={cn(
        ...
        active
          ? 'bg-[var(--dash-surface)] text-[var(--dash-text)] border-[var(--dash-border)]'
          : 'bg-[var(--dash-surface)]/60 text-[var(--dash-text-muted)] border-transparent hover:text-[var(--dash-text-secondary)] hover:bg-[var(--dash-surface)]/80',
      )}
    >
      {icon}
      <span className="text-[9px] font-medium leading-none">{label}</span>
      {showDot && <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-[var(--dash-red)]" />}
    </button>
  )
}
