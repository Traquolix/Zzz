...
import { fetchInfrastructure, fetchSHMStatus, fetchSpectralData, fetchPeakFrequencies, fetchSpectralSummary } from '@/api/infrastructure'
...
export function useStructures(selectedId: string | null) {
    ...
    useEffect(() => {
        ...
        fetchInfrastructure()
            .then(data => {
                ...
                Promise.allSettled(data.map(s =>
                    fetchSHMStatus(s.id).then(status => [s.id, status] as const)
                )).then(results => {
                    if (cancelled) return
    ...
    useEffect(() => {
        ...
        fetchSHMStatus(selectedId)
            .then(data => { if (!cancelled) setShmStatus(data) })
            .catch(() => { if (!cancelled) setShmStatus(null) })

        // Spectral data (demo mode — no infrastructureId, the backend serves sample HDF5 data)
        setSpectralLoading(true)
        fetchSpectralData()
            .then(data => { if (!cancelled) setSpectralData(data) })
            .catch(() => { if (!cancelled) setSpectralData(null) })
            .finally(() => { if (!cancelled) setSpectralLoading(false) })

        // Peak frequencies (demo mode)
        setPeakLoading(true)
        fetchPeakFrequencies()
            .then(data => { if (!cancelled) setPeakData(data) })
            .catch(() => { if (!cancelled) setPeakData(null) })
            .finally(() => { if (!cancelled) setPeakLoading(false) })

        // Summary
        fetchSpectralSummary()
            .then(data => { if (!cancelled) setDataSummary(data) })
            .catch(() => { if (!cancelled) setDataSummary(null) })
