...
export function useLiveStats(sections: Section[]) {
    ...
    const [stats, setStats] = useState<Map<string, LiveSectionStats>>(
        () => new Map(),
    )
    const statsRef = useRef<Map<string, LiveSectionStats>>(new Map())
    const [seriesData, setSeriesData] = useState<Map<string, SectionDataPoint[]>>(
        () => new Map(),
    )
