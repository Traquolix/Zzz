...
import type { ProtoState, ProtoAction, Severity, MetricKey, Section, LiveSectionStats, SectionDataPoint, SpeedThresholds, SelectedChannel } from '../types'
...
export function SidePanel({ state, dispatch, connected, liveStats, liveSeriesData, onHighlightFiber, onHighlightSection, onHighlightIncident, onClearHighlight, structureData, unseenIds, hasUnseen, onMarkSeen }: SidePanelProps) {
    const { activeTab, selectedIncidentId, selectedSectionId, selectedStructureId, selectedChannel, filterSeverity, hideResolved, sectionMetric, sections, incidents, sidebarOpen, fiberColors, showStructuresOnMap, showStructureLabels, showIncidentsOnMap } = state
        ...
        <div className="relative h-full">
            ...
            >
                            ...
                            <>
                                {filterSeverity && (
                                    <button
                                        ...
                                        title="Clear filter"
                                    >
                                        <svg width="10" height="10" viewBox="0 0 10 10" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" className="text-[var(--proto-surface)]">
                                ...
                                {severityOrder.map(s => (
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            filterSeverity === s ? 'ring-1 ring-[var(--proto-text-secondary)] scale-125' : 'opacity-50 hover:opacity-80',
                                        )}
                                ...
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        hideResolved ? 'text-[var(--proto-text-muted)] hover:text-[var(--proto-text-secondary)]' : 'text-[var(--proto-accent)]',
                                    )}
                                ...
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        showIncidentsOnMap ? 'text-[var(--proto-accent)]' : 'text-[var(--proto-text-muted)] hover:text-[var(--proto-text-secondary)]',
                                    )}
                        ...
                        {activeTab === 'shm' && (
                            <>
                                <div className="relative">
                                    <svg className="absolute left-1.5 top-1/2 -translate-y-1/2 pointer-events-none text-[var(--proto-text-muted)]" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                                ...
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        showStructuresOnMap ? 'text-[var(--proto-accent)]' : 'text-[var(--proto-text-muted)] hover:text-[var(--proto-text)]',
                                    )}
                                    title="Show on map"
                                >
                                ...
                                <button
                                    ...
                                    className={cn(
                                        'flex items-center justify-center w-6 h-6 rounded transition-colors cursor-pointer',
                                        showStructureLabels ? 'text-[var(--proto-accent)]' : 'text-[var(--proto-text-muted)] hover:text-[var(--proto-text)]',
                                    )}
                                    title="Show labels"
                                >
                                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                ...
                <div className="flex-1 overflow-y-auto">
                    {activeTab === 'incidents' && (
                        incident
                            ? <IncidentDetail incident={incident} sections={sections} dispatch={dispatch} onBack={() => dispatch({ type: 'CLEAR_SELECTION' })} />
                            : <IncidentList incidents={incidents} filterSeverity={filterSeverity} hideResolved={hideResolved} sortBy={incidentSortBy} dispatch={dispatch} onHighlightIncident={onHighlightIncident} onClearHighlight={onClearHighlight} unseenIds={unseenIds} onMarkSeen={onMarkSeen} />
                    )}
                    {activeTab === 'sections' && (
                        section
                            ? <SectionDetail section={section} onBack={() => dispatch({ type: 'CLEAR_SELECTION' })} liveStats={liveStats} liveSeriesData={liveSeriesData} dispatch={dispatch} fiberColors={fiberColors} />
...
function TabButton({ label, icon, active, onClick, showDot }: { label: string; icon: React.ReactNode; active: boolean; onClick: () => void; showDot?: boolean }) {
    return (
        <button
            onClick={onClick}
            className={cn(
        ...
        >
            ...
            {showDot && (
                <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-[var(--proto-red)]" />
            )}
        </button>
    )
}
...
function ChannelDetail({ channel, sections, dispatch, fiberColors }: {
    channel: SelectedChannel
    sections: Section[]
    dispatch: React.Dispatch<ProtoAction>
        ...
        <div className="proto-analysis-enter flex flex-col">
            ...
            <div className="sticky top-0 z-10 bg-[var(--proto-surface)] border-b border-[var(--proto-border)] px-4 py-3">
                <div className="min-w-0">
                    <span className="text-sm font-semibold text-[var(--proto-text)] truncate block">Channel {channel.channel}</span>
            ...
            <div className="px-4 py-4 flex flex-col gap-3">
                ...
                <div className="rounded-lg border border-[var(--proto-border)] overflow-hidden">
                    ...
                    <canvas
                        ref={canvasRef}
                        className="w-full"
                        style={{ height: 160, borderRadius: '0 0 8px 8px' }}
                    />
                </div>
...
function IncidentList({ incidents, filterSeverity, hideResolved, sortBy, dispatch, onHighlightIncident, onClearHighlight, unseenIds, onMarkSeen }: { incidents: Incident[]; filterSeverity: Severity | null; hideResolved: boolean; sortBy: 'newest' | 'oldest'; dispatch: React.Dispatch<ProtoAction>; onHighlightIncident?: (id: string) => void; onClearHighlight?: () => void; unseenIds?: Set<string>; onMarkSeen?: (id: string) => void }) {
                ...
                <div className="flex flex-col px-3 py-1">
                    {sorted.map(inc => (
                        <button
                            ...
                            onMouseEnter={() => { onHighlightIncident?.(inc.id); onMarkSeen?.(inc.id) }}
                        ...
                        >
                            <div className="flex items-start gap-2.5 min-w-0">
                                <span className="shrink-0 w-2 h-2 rounded-full mt-1.5" style={{ backgroundColor: severityColor[inc.severity] }} />
                                ...
                                <div className="min-w-0 flex-1">
                                    ...
                                    <div className="flex items-center gap-1.5 text-[11px] text-[var(--proto-text-muted)] mt-0.5">
                                        <span>Ch {inc.channel}{inc.channelEnd && inc.channelEnd !== inc.channel ? `–${inc.channelEnd}` : ''}</span>
...
function SectionList({ sections, dispatch, liveStats, liveSeriesData, metric, fiberColors, onHighlightSection, onClearHighlight }: { sections: Section[]; dispatch: React.Dispatch<ProtoAction>; liveStats: Map<string, LiveSectionStats>; liveSeriesData: Map<string, SectionDataPoint[]>; metric: MetricKey; fiberColors: Record<string, string>; onHighlightSection?: (id: string) => void; onClearHighlight?: () => void }) {
                ...
                <div className="flex flex-col px-3 py-1">
                    {sections.map(section => {
                        ...
                        if (metric === 'speed') {
                            ...
                            spark = liveSeries?.length
                                ? liveSeries.slice(-SPARK_LENGTH).map(p => p.speed)
                                : section.speedHistory
                        } else if (metric === 'flow') {
                            displayValue = live?.flow ?? section.flow
                            spark = liveSeries?.length
                                ? liveSeries.slice(-SPARK_LENGTH).map(p => p.flow)
                                : section.countHistory
                        } else {
                            displayValue = live?.occupancy ?? section.occupancy
                            spark = liveSeries?.length
                                ? liveSeries.slice(-SPARK_LENGTH).map(p => p.occupancy)
                                : []
                        }
                            ...
                            <div key={section.id} className="group relative">
                                ...
                                >
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className="shrink-0 w-2 h-2 rounded-full" style={{ backgroundColor: fiberColors[section.fiberId] ?? fiber?.color }} />
                                    ...
                                    <div className="flex items-center justify-between text-xs text-[var(--proto-text-secondary)] pl-4">
                                        <span>
                                            <span style={{ color: metric === 'speed' ? getSpeedColor(displayValue, section.speedThresholds) : metricConfig.color }}>
                                                {displayValue}
                                            </span>
                                            {' '}{metricConfig.unit}
                                ...
                                <button
                                    onClick={(e) => { e.stopPropagation(); dispatch({ type: 'DELETE_SECTION', id: section.id }) }}
...
function IncidentDetail({ incident, sections, dispatch, onBack }: {
    incident: Incident
    sections: Section[]
    dispatch: React.Dispatch<ProtoAction>
    onBack: () => void
    ...
    const relatedSection = sections.find(
        s => s.fiberId === incident.fiberId &&
            incident.channel >= s.startChannel &&
            incident.channel <= s.endChannel
    )

    // Fetch snapshot data from API
    const [snapshotData, setSnapshotData] = useState<{ time: string; speed: number; flow: number; occupancy: number }[] | null>(null)
    ...
    useEffect(() => {
        ...
        fetchIncidentSnapshot(incident.id)
            ...
            .catch(() => {
                if (mounted) { setSnapshotData(null); setSnapshotLoading(false) }
            })
    ...
    return (
        <div className="proto-analysis-enter flex flex-col">
            <div className="sticky top-0 z-10 bg-[var(--proto-surface)] border-b border-[var(--proto-border)] px-4 py-3 flex items-center gap-3">
                <button onClick={onBack} className="text-[var(--proto-text-muted)] hover:text-[var(--proto-text)] transition-colors text-sm cursor-pointer">
                    &larr; Back
                </button>
            ...
            <div className="px-4 py-4 flex flex-col gap-3">
                    ...
                    <div className="grid grid-cols-3 gap-2 pb-3 border-b border-[var(--proto-border)]">
                        {incident.speedBefore != null && (
                            <div className="rounded-lg border border-[var(--proto-border)] p-2.5">
                                ...
                                <span className="text-lg font-semibold text-[var(--proto-text)]">{Math.round(incident.speedBefore)}</span>
                        ...
                        {incident.speedDuring != null && (
                            <div className="rounded-lg border border-[var(--proto-border)] p-2.5">
                                ...
                                <span className="text-lg font-semibold text-[var(--proto-red)]">{Math.round(incident.speedDuring)}</span>
                                <span className="text-[10px] text-[var(--proto-text-muted)] ml-0.5">km/h</span>
                            </div>
                        )}
                        {incident.speedDropPercent != null && (
                            <div className="rounded-lg border border-[var(--proto-border)] p-2.5">
                                ...
                                <span className="text-lg font-semibold text-[var(--proto-red)]">{Math.round(incident.speedDropPercent)}</span>
                ...
                <div className="pb-3 border-b border-[var(--proto-border)]">
                    {editing ? (
                        <div className="flex flex-col gap-2">
                            ...
                            <div className="flex gap-2 justify-end">
                                <button
                                    onClick={() => { setDraft(incident.description); setEditing(false) }}
                                    className="px-2 py-1 rounded text-xs text-[var(--proto-text-muted)] hover:text-[var(--proto-text)] transition-colors cursor-pointer"
                                >
                                    Cancel
                                </button>
                                <button
                                    onClick={() => { dispatch({ type: 'UPDATE_INCIDENT_DESCRIPTION', id: incident.id, description: draft }); setEditing(false) }}
                        ...
                        <div
                            ...
                            onClick={() => { setDraft(incident.description); setEditing(true) }}
                            title="Click to edit"
                        >
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-[var(--proto-text-secondary)]">
                        ...
                        <span>Channel: {incident.channel}{incident.channelEnd != null && incident.channelEnd !== incident.channel ? `–${incident.channelEnd}` : ''}</span>
                        <span>
                            Status:{' '}
                ...
                <div>
                    ...
                    {snapshotLoading ? (
                        <div className="text-xs text-[var(--proto-text-muted)] animate-pulse py-4 text-center">Loading snapshot...</div>
                    ) : snapshotData ? (
                        <TimeSeriesChart data={snapshotData} />
                    ) : (
                        <div className="text-xs text-[var(--proto-text-muted)] italic py-4 text-center">No snapshot data available</div>
                    )}
                </div>
            </div>
        </div>
...
function ThresholdEditor({ thresholds, onChange }: { thresholds: SpeedThresholds; onChange: (t: SpeedThresholds) => void }) {
    const [draft, setDraft] = useState<SpeedThresholds>(thresholds)
    const isDirty = draft.green !== thresholds.green || draft.yellow !== thresholds.yellow || draft.orange !== thresholds.orange

    // Sync draft when thresholds change externally (e.g. switching sections)
    useEffect(() => { setDraft(thresholds) }, [thresholds.green, thresholds.yellow, thresholds.orange])
...
function ColorPicker({ current, onSelect, onClose, anchorRef }: {
    current: string
...
function FiberColorDot({ direction, color, isPickerOpen, onTogglePicker, onSelect, onClosePicker, onMouseEnter, onMouseLeave }: {
    direction: 0 | 1
    color: string
...
function SettingsPanel({ fiberThresholds, fiberColors, dispatch, onHighlightFiber, onClearHighlight, show3DBuildings }: {
    ...
    const cableGroups = useMemo(() => {
        ...
        for (const f of fibers) {
            ...
            if (!group) { group = { name: f.name, fibers: [] }; map.set(f.parentCableId, group) }
...
function SectionDetail({ section, onBack, liveStats, liveSeriesData, dispatch, fiberColors }: {
    section: Section
    onBack: () => void
    liveStats: Map<string, LiveSectionStats>
    liveSeriesData: Map<string, SectionDataPoint[]>
    ...
    const SPARK_LENGTH = 30
    const speedSpark = liveSeries?.length
        ? liveSeries.slice(-SPARK_LENGTH).map(p => p.speed)
        : section.speedHistory
    const flowSpark = liveSeries?.length
        ? liveSeries.slice(-SPARK_LENGTH).map(p => p.flow)
        : section.countHistory
    const occupancySpark = liveSeries?.length
        ? liveSeries.slice(-SPARK_LENGTH).map(p => p.occupancy)
        : null
    ...
    const kpis = [
        { label: 'Avg Speed', value: `${displaySpeed}`, unit: 'km/h', trend: speedSpark, color: '#6366f1', trendPct: speedTrend.pct, positiveIsGood: true },
        { label: 'Flow', value: `${displayFlow}`, unit: 'veh/min', trend: flowSpark, color: '#8b5cf6', trendPct: flowTrend.pct, positiveIsGood: true },
    ...
    const [historyData, setHistoryData] = useState<{ time: string; speed: number; flow: number; occupancy: number }[] | null>(null)
    ...
    useEffect(() => {
        ...
        fetchSectionHistory(section.id, 60)
            ...
            .catch(() => { if (mounted) setHistoryData(null) })
        return () => { mounted = false }
            ...
            <div className="sticky top-0 z-10 bg-[var(--proto-surface)] border-b border-[var(--proto-border)] px-4 py-3 flex items-center gap-3">
                ...
                <div className="min-w-0">
                    ...
                    {fiber && (
                        <span className="text-[10px] text-[var(--proto-text-muted)] flex items-center gap-1.5">
                            <span className="inline-block w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: fiberColor }} />
                            {fiber.name} · {fiber.direction === 0 ? 'Dir A' : 'Dir B'} · Ch {section.startChannel}–{section.endChannel}
                        </span>
                    )}
                </div>
            </div>
                    ...
                    {kpis.map(kpi => (
                        <div key={kpi.label} className="rounded-lg border border-[var(--proto-border)] p-3">
                            <div className="text-[10px] text-[var(--proto-text-muted)] uppercase tracking-wider mb-1">
                                {kpi.label}
                                {kpi.trendPct !== undefined && <TrendBadge pct={kpi.trendPct} positiveIsGood={kpi.positiveIsGood ?? true} />}
                            </div>
...
function StructureList({ structures, loading, allStatuses, search, dispatch, onHighlightSection, onClearHighlight }: {
    structures: Infrastructure[]
    loading: boolean
    allStatuses: Map<string, SHMStatus>
    search: string
    ...
    const filtered = q
        ? structures.filter(s => {
            ...
            return s.name.toLowerCase().includes(q)
                ...
                || fiberName.toLowerCase().includes(q)
        })
        : structures
                    ...
                    >
                        ...
                        {structure.imageUrl ? (
                            ...
                            >
                                {structure.type === 'bridge' ? (
                                    <svg width="28" height="28" viewBox="0 0 16 16" fill="none" stroke={typeStyle.text} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                                        <path d="M1 12h14" />
                                        <path d="M3 12V7" />
                                        <path d="M13 12V7" />
                                        <path d="M3 7C3 7 5.5 4 8 4C10.5 4 13 7 13 7" />
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <span className="text-xs text-[var(--proto-text-muted)] shrink-0">
                                {structure.type.charAt(0).toUpperCase() + structure.type.slice(1)} · {fiber?.name ?? structure.fiberId}
                            </span>
                        </div>
                    </button>
                )
...
function StructureDetail({ structure, shmStatus, spectralData, spectralLoading, peakData, peakLoading, dataSummary, onBack }: {
        ...
        <div className="proto-analysis-enter flex flex-col">
            ...
            <div className="sticky top-0 z-10 bg-[var(--proto-surface)] border-b border-[var(--proto-border)] px-4 py-3 flex items-center gap-3">
                ...
                <div className="min-w-0">
                    ...
                    {fiber && (
                        <span className="text-[10px] text-[var(--proto-text-muted)] flex items-center gap-1.5">
                            <span className="inline-block w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: typeStyle.dot }} />
            ...
            <div className="px-4 py-4 flex flex-col gap-3">
                ...
                {comparisonStats && (
                    <div className="flex items-center justify-between rounded-lg border border-[var(--proto-border)] bg-[var(--proto-surface-raised)] px-4 py-3">
                        <div>
                            <span className={`text-xl font-bold ${comparisonStats.diff > 0 ? 'text-green-400' : comparisonStats.diff < 0 ? 'text-red-400' : 'text-[var(--proto-text)]'}`}>
                                {comparisonStats.diff > 0 ? '+' : ''}{(comparisonStats.diff * 1000).toFixed(2)} mHz
                            </span>
                            <div className="flex items-center gap-2 mt-0.5">
                                <span className={`text-xs ${comparisonStats.diff > 0 ? 'text-green-500' : comparisonStats.diff < 0 ? 'text-red-500' : 'text-[var(--proto-text-muted)]'}`}>
                                    ({comparisonStats.diffPercent > 0 ? '+' : ''}{comparisonStats.diffPercent.toFixed(2)}%)
                                </span>
...
function PeakScatterPlot({ data }: { data: PeakFrequencyData }) {
    ...
    const brushRect = brush ? { x: Math.min(brush.startX, brush.currentX), width: Math.abs(brush.currentX - brush.startX) } : null
        ...
        <div ref={containerRef} className="w-full relative">
            ...
            <div className="overflow-hidden">
                <svg
                    ...
                    onMouseLeave={() => { if (brush) setBrush(null) }}
                ...
                >
                    ...
                    {/* Y-axis */}
                    <line x1={padding.left} y1={padding.top} x2={padding.left} y2={height - padding.bottom} stroke="rgba(255,255,255,0.08)" strokeWidth={1} />
                    {yTicks.map(tick => (
                        <g key={tick}>
                            <line x1={padding.left - 3} y1={yScale(tick)} x2={padding.left} y2={yScale(tick)} stroke="#64748b" strokeWidth={1} />
                            <text x={padding.left - 6} y={yScale(tick)} textAnchor="end" dominantBaseline="middle" fill="#64748b" fontSize="10">
                                {tick.toFixed(2)}
                            </text>
                        </g>
                    ))}
                    <text x={12} y={height / 2} textAnchor="middle" dominantBaseline="middle" transform={`rotate(-90, 12, ${height / 2})`} fill="#64748b" fontSize="9">
                        Peak Freq (Hz)
                    </text>

                    {/* X-axis */}
                    <line x1={padding.left} y1={height - padding.bottom} x2={width - padding.right} y2={height - padding.bottom} stroke="rgba(255,255,255,0.08)" strokeWidth={1} />
                    {xTicks.map((tick, i) => (
                        <g key={i}>
                            <line x1={tick.x} y1={height - padding.bottom} x2={tick.x} y2={height - padding.bottom + 3} stroke="#64748b" strokeWidth={1} />
                    ...
                    {/* Grid lines */}
                    {yTicks.map(tick => (
                        <line key={`g-${tick}`} x1={padding.left + 1} y1={yScale(tick)} x2={width - padding.right} y2={yScale(tick)} stroke="rgba(255,255,255,0.03)" strokeWidth={1} />
                    ))}
                    ...
                    <g clipPath={`url(#${clipId})`}>
                        {points.filter(pt => pt.inRange && pt.inTimeRange).map((pt, i) => (
                            <circle
                                ...
                                onMouseEnter={(e) => { e.stopPropagation(); if (!brush) setTooltip({ x: pt.x, y: pt.y, freq: pt.freq, power: pt.power, timestamp: pt.timestamp }) }}
                    ...
                    {brushRect && (
                        <rect
                            x={brushRect.x} y={padding.top} width={brushRect.width} height={plotH}
                            fill="var(--proto-accent)" fillOpacity={0.15} stroke="var(--proto-accent)" strokeWidth={1} pointerEvents="none"
                        />
                    )}
                </svg>
            </div>
            ...
            {tooltip && !brush && (
                ...
                >
                    ...
                    <div className="text-[var(--proto-text-muted)]">
                        {tooltip.timestamp.toLocaleString(undefined, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                    </div>
                </div>
            )}
        </div>
...
function ComparisonOverlay({ dataA, dataB, focus, width }: {
    ...
    const freqMin = 1.05, freqMax = 1.20
    ...
    return (
        <svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`} className="overflow-visible" preserveAspectRatio="xMidYMid meet">
            ...
            {yTicks.map(tick => (
                <g key={tick}>
                    <line x1={padding.left - 3} y1={yScale(tick)} x2={padding.left} y2={yScale(tick)} stroke="#64748b" strokeWidth={1} />
                    <text x={padding.left - 6} y={yScale(tick)} textAnchor="end" dominantBaseline="middle" fill="#64748b" fontSize="10">{tick.toFixed(2)}</text>
                    <line x1={padding.left + 1} y1={yScale(tick)} x2={width - padding.right} y2={yScale(tick)} stroke="rgba(255,255,255,0.03)" strokeWidth={1} />
                </g>
            ))}
            <text x={12} y={height / 2} textAnchor="middle" dominantBaseline="middle" transform={`rotate(-90, 12, ${height / 2})`} fill="#64748b" fontSize="9">Hz</text>

            {/* X-axis */}
            <line x1={padding.left} y1={height - padding.bottom} x2={width - padding.right} y2={height - padding.bottom} stroke="rgba(255,255,255,0.08)" strokeWidth={1} />
            <text x={padding.left} y={height - 4} textAnchor="start" fill="#4a5568" fontSize="9">start</text>
            <text x={width - padding.right} y={height - 4} textAnchor="end" fill="#4a5568" fontSize="9">end</text>

            {/* Dots: render unfocused behind, focused in front */}
            <g clipPath={`url(#${clipId})`}>
                {focus !== 'A' && pointsB.filter(p => p.inRange).map((pt, i) => (
                    <circle key={`b-${i}`} cx={pt.x} cy={pt.y} r={2.5} fill={pt.color} fillOpacity={opacityB} />
                ))}
                {pointsA.filter(p => p.inRange).map((pt, i) => (
...
type ComparisonStats = { a: { mean: number; std: number; count: number }; b: { mean: number; std: number; count: number }; diff: number; diffPercent: number }

function ComparisonSection({ dataSummary, onStats }: { dataSummary: SpectralSummary | null; onStats?: (stats: ComparisonStats | null) => void }) {
    const containerRef = useRef<HTMLDivElement>(null)
    const [chartWidth, setChartWidth] = useState(280)
    const [mode, setMode] = useState<ComparisonMode>('day')
    const [focus, setFocus] = useState<FocusMode>('equal')
    const [windowA, setWindowA] = useState<{ data: PeakFrequencyData | null; loading: boolean }>({ data: null, loading: false })
    const [windowB, setWindowB] = useState<{ data: PeakFrequencyData | null; loading: boolean }>({ data: null, loading: false })

    useEffect(() => {
        const el = containerRef.current
        if (!el) return
        const measure = () => { if (el) setChartWidth(Math.max(160, el.clientWidth)) }
        measure()
    ...
    const { rangeA, rangeB, labelA, labelB } = useMemo(() => {
        ...
        if (mode === 'day') {
            ...
            const endA = new Date(latestDay); endA.setHours(23, 59, 59, 999)
            const endB = new Date(prevDay); endB.setHours(23, 59, 59, 999)
            return {
        ...
        const thisMonday = new Date(latestDay); thisMonday.setDate(thisMonday.getDate() + mondayOffset)
        const thisSunday = new Date(thisMonday); thisSunday.setDate(thisSunday.getDate() + 6); thisSunday.setHours(23, 59, 59, 999)
        const lastMonday = new Date(thisMonday); lastMonday.setDate(lastMonday.getDate() - 7)
        const lastSunday = new Date(lastMonday); lastSunday.setDate(lastSunday.getDate() + 6); lastSunday.setHours(23, 59, 59, 999)
    ...
    useEffect(() => {
        ...
        fetchPeakFrequencies({ maxSamples: 5000, startTime: rangeA.from, endTime: rangeA.to })
            .then(data => { if (!cancelled) setWindowA({ data, loading: false }) })
            .catch(() => { if (!cancelled) setWindowA({ data: null, loading: false }) })
    ...
    useEffect(() => {
        ...
        fetchPeakFrequencies({ maxSamples: 5000, startTime: rangeB.from, endTime: rangeB.to })
            .then(data => { if (!cancelled) setWindowB({ data, loading: false }) })
            .catch(() => { if (!cancelled) setWindowB({ data: null, loading: false }) })
        return () => { cancelled = true }
    ...
    const stats = useMemo(() => {
        const calc = (d: PeakFrequencyData | null) => {
            if (!d) return null
            const valid = d.peakFrequencies.filter(f => f >= 1.05 && f <= 1.20)
        ...
        const a = calc(windowA.data), b = calc(windowB.data)
        if (!a || !b) return null
    ...
    return (
        <div>
            ...
            {/* Chart */}
            <div ref={containerRef} className="rounded-lg bg-[var(--proto-surface-raised)] border border-[var(--proto-border)] p-2">
                {isLoading ? (
