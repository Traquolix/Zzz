...
export function StatusBar({ connected, sectionCount, incidentCount }: StatusBarProps) {
    ...
    const avgPing = pingHistory.filter(p => p > 0).length > 0
        ? Math.round(pingHistory.filter(p => p > 0).reduce((a, b) => a + b, 0) / pingHistory.filter(p => p > 0).length)
        : null

    const pingColor = lastPing === null ? 'var(--proto-text-muted)'
        : lastPing < 0 ? 'var(--proto-red)'
        : lastPing < 100 ? 'var(--proto-green)'
        : lastPing < 300 ? 'var(--proto-amber)'
        : 'var(--proto-red)'

    return (
        <div className="absolute top-4 left-4 z-10 flex items-center gap-2.5">
            <span className="text-sm font-semibold text-[var(--proto-text)] tracking-tight">
                Sequoia Analytics
            </span>
            <div
                className="relative"
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <div>
                                <div className="flex items-center gap-1.5 mb-1">
                                    <span className={`w-1.5 h-1.5 rounded-full`} style={{ backgroundColor: connected ? 'var(--proto-green)' : 'var(--proto-red)' }} />
                                ...
                                {avgPing != null && (
                                    <div className="text-[var(--proto-text-muted)]/60 mt-0.5">avg {avgPing}ms</div>
                                )}
                            </div>
