...
_PreferencesResponse = inline_serializer('UserPreferencesResponse', fields={
...
class UserPreferencesView(APIView):
    ...
    @extend_schema(
        request=inline_serializer('UserPreferencesRequest', fields={
    ...
    def put(self, request):
        ...
        if payload_size > MAX_PREFERENCES_SIZE:
            return Response(
                {'detail': f'Payload too large ({payload_size} bytes, max {MAX_PREFERENCES_SIZE}).', 'code': 'payload_too_large'},
                status=400,
            )
        ...
        if validation_errors:
            return Response(
                {'detail': 'Invalid preferences structure', 'errors': validation_errors[:10], 'code': 'validation_error'},
                status=400,
            )
