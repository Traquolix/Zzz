...
class LoginView(APIView):
    ...
    def post(self, request):
        ...
        if attempts >= LOCKOUT_MAX_ATTEMPTS:
            ...
            return Response(
                {'detail': 'Account temporarily locked. Try again later.', 'code': 'account_locked'},
        ...
        if user is not None and user.is_active:
            ...
            if remaining <= 0:
                logger.warning('Account locked after %d failed attempts: %s', LOCKOUT_MAX_ATTEMPTS, username)
            elif remaining <= 2:
                logger.info('Login attempt %d/%d for: %s', attempts + 1, LOCKOUT_MAX_ATTEMPTS, username)
...
class VerifyView(APIView):
    ...
    @extend_schema(
        responses={200: inline_serializer('VerifyResponse', fields={
