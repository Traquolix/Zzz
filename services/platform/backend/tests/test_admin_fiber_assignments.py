...
class TestFiberAssignmentViews(TestCase):
    ...
    def test_create_fiber_assignment_superuser(self):
        ...
        # Verify in DB
        assert FiberAssignment.objects.filter(
            organization=self.org, fiber_id='new-fiber'
        ).exists()
    ...
    def test_create_fiber_assignment_nonexistent_org(self):
        ...
        resp = self.client.post(
            f'/api/admin/organizations/{fake_org_id}/fibers',
            {'fiberId': 'fiber-1'},
            format='json'
        )
    ...
    def test_delete_fiber_assignment_superuser(self):
        ...
        resp = self.client.delete(
            f'/api/admin/organizations/{self.org.pk}/fibers/{assignment.pk}'
        )
    ...
    def test_delete_assignment_from_wrong_org(self):
        ...
        assignment = FiberAssignment.objects.create(organization=self.other_org, fiber_id='other-fiber')
        self.client.force_authenticate(user=self.superuser)
        resp = self.client.delete(
            f'/api/admin/organizations/{self.org.pk}/fibers/{assignment.pk}'
        )
