...
class TestReadinessCheck:
    ...
    def test_readiness_clickhouse_ok(self, mock_get_client, mock_ch_health, api_client):
        ...
        mock_ch_health.return_value = {'consecutive_failures': 0, 'in_cooldown': False, 'last_failure': 0}
    ...
    @patch('apps.shared.clickhouse.health')
    @patch('apps.shared.clickhouse.get_client')
    def test_readiness_degraded_when_clickhouse_in_cooldown(self, mock_get_client, mock_ch_health, api_client):
        """Circuit breaker in cooldown -> clickhouse 'unavailable'."""
        mock_ch_health.return_value = {'consecutive_failures': 5, 'in_cooldown': True, 'last_failure': 0}
        response = api_client.get('/api/health/ready')
    ...
    @patch('apps.shared.clickhouse.health')
    @patch('apps.shared.clickhouse.get_client')
    def test_readiness_degraded_when_clickhouse_query_fails(self, mock_get_client, mock_ch_health, api_client):
