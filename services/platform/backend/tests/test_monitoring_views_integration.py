...
class TestIncidentSnapshotOrgScoping:
    ...
    def test_snapshot_rejected_for_other_org_fiber(self, authenticated_client, fiber_assignments):
        ...
        with patch(
            "apps.monitoring.views.query", return_value=self._mock_incident_row("unknown-fiber:0")
        ):
            resp = authenticated_client.get("/api/incidents/inc-2/snapshot")

        assert resp.status_code == 404

    def test_snapshot_directional_suffix_stripped_for_check(
        self, authenticated_client, fiber_assignments
    ):
...
class TestSnapshotQueryBehavior:
    ...
    def test_snapshot_normalizes_fiberline_in_response(
        self, authenticated_client, fiber_assignments
    ):
...
class TestStatsView:
    ...
    def test_stats_clickhouse_unavailable_returns_503(
        self, authenticated_client, fiber_assignments
    ):
        ...
        with patch(
            "apps.monitoring.views.query_scalar", side_effect=ClickHouseUnavailableError("down")
        ):
            resp = authenticated_client.get("/api/stats")
