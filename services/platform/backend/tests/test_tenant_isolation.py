...
from apps.fibers.utils import get_org_fiber_ids, get_fiber_org_map
...
class TestFiberAssignment:
    ...
    def test_get_fiber_org_map(self, org, other_org, fiber_assignments, other_org_fiber_assignments):
...
class TestUserWidgetInheritance:
    ...
    def test_admin_gets_all_by_default(self):
        ...
        from apps.shared.constants import ALL_WIDGETS, ALL_LAYERS
    ...
    def test_viewer_gets_viewer_defaults(self):
        ...
        from apps.shared.constants import VIEWER_WIDGETS, VIEWER_LAYERS
...
class TestPostgresIsolation:
    ...
    def test_preferences_isolation(self, authenticated_client, other_org_client, admin_user, other_org_user):
...
class TestFiberEndpointIsolation:
    ...
    @patch('apps.fibers.views.get_client')
    def test_org_b_only_sees_own_fibers(
        self, mock_get_client, other_org_client, other_org
    ):
    ...
    @patch('apps.fibers.views.get_client')
    def test_no_assignments_skips_clickhouse_entirely(
...
class TestIncidentEndpointIsolation:
    ...
    def test_snapshot_cross_org_rejected(
        self, mock_query, other_org_client, other_org
    ):
        """
    ...
    @patch('apps.monitoring.views.query')
    def test_snapshot_same_org_allowed(
        self, mock_query, authenticated_client, org
    ):
        """
        ...
        mock_query.side_effect = [
            # Incident lookup
            [{'fiber_id': 'carros', 'channel_start': 100, 'channel_end': 110, 'timestamp_ns': 1717200000000000000}],
            # Speed hires data
            [],
        ]
...
class TestStatsEndpointIsolation:
    ...
    def test_stats_parameterized_with_org_fibers(
        self, mock_scalar, authenticated_client, org
    ):
...
class TestSuperuserBypass:
    ...
    def test_superuser_snapshot_any_fiber(self, mock_query, superuser_client):
        ...
        mock_query.side_effect = [
            [{'fiber_id': 'unassigned-fiber', 'channel_start': 50, 'channel_end': 60, 'timestamp_ns': 1717200000000000000}],
            [],  # speed data
        ]
...
class TestCacheIsolation:
    ...
    def test_fiber_cache_per_org(
        ...
        mock_result_a.named_results.return_value = [{
            'fiber_id': 'carros', 'fiber_name': 'Carros',
            'color': '#ff0000', 'channel_coordinates': [(7.1, 43.7)],
            'landmark_labels': [None],
        }]
        # Second call: org B sees promenade
        mock_result_b = MagicMock()
        mock_result_b.named_results.return_value = [{
            'fiber_id': 'promenade', 'fiber_name': 'Promenade',
            'color': '#00ff00', 'channel_coordinates': [(7.25, 43.69)],
            'landmark_labels': [None],
        }]
