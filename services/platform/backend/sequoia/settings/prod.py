...
for _name, _value in _required_settings.items():
    if not _value:
        raise ImproperlyConfigured(
            f"Production setting {_name} is empty. Check your environment variables."
        )

if "*" in ALLOWED_HOSTS:
    raise ImproperlyConfigured("ALLOWED_HOSTS contains '*', which is not safe for production.")
