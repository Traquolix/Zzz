...
# ---------------------------------------------------------------------------
# JWT RS256 dev keys — loaded from .env.dev or environment
# Generate with: openssl genrsa 2048 | tee jwt_private.pem | openssl rsa -pubout > jwt_public.pem
# Then set JWT_DEV_PRIVATE_KEY / JWT_DEV_PUBLIC_KEY in .env.dev (use \n for newlines)
# ---------------------------------------------------------------------------
...
if not _DEV_JWT_PRIVATE_KEY or not _DEV_JWT_PUBLIC_KEY:
    raise RuntimeError(
        'JWT_DEV_PRIVATE_KEY and JWT_DEV_PUBLIC_KEY must be set in .env.dev or environment.\n'
        'Generate with: openssl genrsa 2048 | tee jwt_private.pem | openssl rsa -pubout > jwt_public.pem\n'
        'Then paste as single-line with \\n separators into .env.dev'
    )
