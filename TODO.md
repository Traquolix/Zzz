...
- [ ] **Centralize aggregation in backend** — the frontend currently computes occupancy, rolling averages, and per-second bucketing for section live stats (`useLiveStats.ts`) and formerly for snapshots. Move all aggregation math (avg speed, flow, occupancy formula) to the backend so the frontend only renders pre-computed points. Eliminates duplicated `AVG_VEHICLE_LENGTH` constants and occupancy formulas across frontend/backend.
