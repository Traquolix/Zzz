...
- [x] ~~**Fix incident snapshot data** — fixed: rolling detection buffer seeds ±60s window around incident, frontend groups by second and polls every 1s until complete.~~
- [ ] **Incident replay player** — interactive playback of incident snapshots: time slider, play/pause, map visualization of detection dots color-coded by speed, chart cursor sync. See [`TODO/plans/incident-player.md`](TODO/plans/incident-player.md)
